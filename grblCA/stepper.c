...
ISR(TIMER1_COMPA_vect)
{
  ...
  set_pins(st.dir_outbits); // Custom direction pins reading
...
// Then pulse the stepping pins
  ...
  set_pins(st.step_outbits); // Custom step pins reading
#endif
...
ISR(TIMER0_OVF_vect)
{
  ...
  set_pins(step_port_invert_mask & STEP_MASK); // Custom
...
void st_reset()
{
  ...
  set_pins(step_port_invert_mask); // Custom
  set_pins(dir_port_invert_mask);  // Custom
}
